...
crps_dt <- rbind(crps_dt,
                 data.table(k=width,
                   k.specified=width,
                   mcrps=mean(results$mcrps)))
...
fwrite(crps_dt, file='C:/Users/gpitt/Documents/GitHub/conditional-aj-reserving/results_csv/crpss.csv')
...
# 
# imx[1:(width-1),] <- imx[1:(width-1),]/jmx[1:(width-1),]
# # imx[1:(width-2),] <- imx[1:(width-2),]/jmx[1:(width-2),]
# apply(imx,1,sum)
# 
# lambda <- function(t){
#   A <- matrix(c(1*mark_dist(1, t, 0),
...
#   # data.setC[,"X"] <- X
#   
#   data.list2 <- lapply(data.list,reformulate_state_space,newk = 7)
#   
#   fit <- AalenJohansen::aalen_johansen(data.list2)
#   yhat <-  sapply(fit$p, last)
#   # cond <- yhat <=1 & yhat >=0
...
# 
# 
# foo <- matrix(as.array(crps_dt$x), length(crps_dt$x),3, byrow = TRUE)
# 
# 
# fwrite(as.data.table(foo),file="C:\\Users\\gpitt\\Documents\\GitHub\\conditional-aj-reserving\\results_csv\\crpss.csv")
# 
# 
# 
